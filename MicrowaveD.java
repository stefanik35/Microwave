...
//microwaveD class
public class MicrowaveD extends Microwave
{
    ...
    private JButton timeButton2, powerButton2;

    public MicrowaveD()
    {
        //set colors and fonts
        backgroundColor = new Color(35, 66, 45);
        foregroundColor = new Color(40, 178, 75);
        identifierFont = new Font("SimSun", Font.BOLD + Font.ITALIC, 24);
        label1Font = new Font("SimSun", Font.BOLD, 20);
        label2Font = new Font("SimSun", Font.ITALIC, 16);
        outputFont = new Font("SimSun", Font.PLAIN, 24);
        buttonFont = new Font("SimSun", Font.BOLD, 16);
        ...
        identifierLabel = new JLabel("Distraught Daphne", SwingConstants.CENTER);
        ...
        powerButton2.setFont(buttonFont);
        powerButton2.setForeground(backgroundColor);
        powerButton2.setBackground(foregroundColor);
        inputPanel.add(powerButton2);
        powerButton2.addActionListener(this);
        ...
        timeButton2 = new JButton("Time -");
        timeButton2.setFont(buttonFont);
        timeButton2.setForeground(backgroundColor);
        timeButton2.setBackground(foregroundColor);
        inputPanel.add(timeButton2);
    ...
    public void actionPerformed(ActionEvent ae)
    {
        ...
        if (button == timeButton2)
        {
            ...
            if (timeButtons.getSelection().getActionCommand().equals("minutes"))
            {
                ...
                //reset the minutes to 59 if it is < 0
                if (minutesText < 0)
                {
                    minutesText = 59;
                }
            } else
            {
                ...
                //reset the seconds to 59 if it is < 0
                if (secondsText < 0)
                {
                    secondsText = 59;
                }
        ...
        else if (button == powerButton2)
        {
            //get information from the gui
            Integer powerText = Integer.parseInt(powerDisplay.getText().substring(0, powerDisplay.getText().length() - 1));

            //increment the power level
            powerText += 10;

            //reset the power to 0 if it is > 100
            if (powerText > 100)
            {
                powerText = 0;
            }
